...
def us_in(board,turn): # Board is the gameboard ( list of lists ) and turn is the played whose turn it is ( 1 or 2 ).
	end = False
	while True:
		while True:
			try: # getting user inpur and verifying the format "int,int"
				us_in_raw = input('Please enter the position you would like to place your symbol on in format "Horizontal,Vertical" or enter "end" to quit.')
				if us_in_raw == 'end':
					end = True
				us_in_list = us_in_raw.split(",")
				h = int(us_in_list[0])
				v = int(us_in_list[1])
			except:
				if not end:
					print('Something went wrong with your input. An example of what you are supposed to enter is "1,1".')
			else:
				break
			finally:
				if end:
		...
		if h > len(board) or v > len(board): # verifying that the entered integers are not greater than the board dimensions. 
			print('Your position is not present on the board.')
		elif h < 1 or v < 1: # verify that the position entered does not contain 0s and negatives.
			print('No zeros, No negarives!')
		elif board[h-1][v-1] != 0: # Check if the given position is available.
...
def new_board(size):
	board = [[0 for x in range(size)] for i in range(size)]
...
if __name__ == '__main__': # test
	...
	full_board(game) # will quit if the board is full

	print(us_in(new_board(5),2)) # will not execute if board is full
